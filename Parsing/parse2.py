...
class EarleyChart:
    ...
    def __init__(self, tokens: List[str], grammar: Grammar, progress: bool = False) -> None:
        ...
        terminal_set = set(self.tokens)
        rules_without_terminal = set()
        for lhs in self.grammar._expansions:
            for rule in self.grammar._expansions[lhs]:
                for symbol in rule.rhs:
                    if not self.grammar.is_nonterminal(symbol) and symbol not in terminal_set:
                        rules_without_terminal.add(rule)
                        if rule in self.grammar._expansions[lhs]:
                            self.grammar._expansions[lhs].remove(rule)

        self._run_earley()    # run Earley's algorithm to construct self.cols

        for rule in rules_without_terminal:
            lhs = rule.lhs
            self.grammar._expansions[lhs].append(rule)

    def accepted(self) -> Tuple[List[Tuple], float]:
        ...
        for item in self.cols[-1].all():    # the last column
            if (item.rule.lhs == self.grammar.start_symbol   # a ROOT item in this column
                ...
                and item.start_position == 0):               # and started back at position 0
                    return (self.cols[-1]._backpointers[item], self.cols[-1]._weights[item])
        return None   # we didn't find any appropriate item
    ...
    def _predict(self, nonterminal: str, position: int) -> None:
        ...
        for rule in self.grammar.expansions(nonterminal):
            ...
            # give new item backpointers and weight
            # for predict, new items don't have to point to anything
            # weight is just the rule weight
            new_item_bps = []
            new_item_weight = rule.weight
            self.cols[position].push(new_item, new_item_bps, new_item_weight)

            log.debug(f"\tPredicted: {new_item} in column {position}")
            self.profile["PREDICT"] += 1

    def _scan(self, item: Item, position: int) -> bool:
        ...
        if position < len(self.tokens) and self.tokens[position] == item.next_symbol():
            ...
            # give new item backpointers and weight
            # backpointers same as previous column plus backpointer to previous column
            # weight is just the same as previous column
            _, item_bps = self.cols[position]._backpointers[item]
            item_bp = [self.cols[position]._backpointers[item]]
            
            new_item_bps = item_bps + item_bp
            new_item_weight = self.cols[position]._weights[item]
            self.cols[position + 1].push(new_item, new_item_bps, new_item_weight)
    ...
    def _attach(self, item: Item, position: int) -> None:
        ...
        for customer in self.cols[mid].all():  # could you eliminate this inefficient linear search?
            if customer.next_symbol() == item.rule.lhs:
                ...
                # give new item backpointers and weight
                # add current item pointer to customer backpointers
                # weight is just customer weight + item weight
                _, customer_bps = self.cols[mid]._backpointers[customer]
                customer_weight = self.cols[mid]._weights[customer]
                item_bp = [self.cols[position]._backpointers[item]]
                item_weight = self.cols[position]._weights[item]

                new_item_bps = customer_bps + item_bp
                new_item_weight = customer_weight + item_weight
                self.cols[position].push(new_item, new_item_bps, new_item_weight)
...
class Agenda:
    ...
    >>> a.push(3, 3.0, 3)
    >>> a.push(5, 5.0, 3)
    >>> a.push(3, 3.0, 3)   # duplicate ignored
    >>> a
    Agenda([]; [3, 5])
    >>> a.pop()
    3
    ...
    def __init__(self) -> None:
        ...
        self._backpointers: Dict[Item, Tuple[Item, List()]] = {}    # Item: Tuple[Item, List()], need a list for backpointers as each rule has multiple RHS symbols
        self._weights: Dict[Item, float] = {}   # Item: float
    ...
    def push(self, item: Item, backpointers: Tuple[Item, List[Tuple]], weight: float) -> None:
        """Add (enqueue) the item, unless it was previously added."""
        if item not in self._weights:    # O(1) lookup in hash table
            self._items.append(item)
            self._weights[item] = weight
            self._backpointers[item] = (item, backpointers)
        else:
            existing_weight = self._weights[item]
    ...
    with open(args.sentences) as f:
        for sentence in f.readlines():
            ...
            if sentence != "":  # skip blank lines
                ...
                # print the result
                result = chart.accepted()
                if result == None:
                    print("NONE")
                else:
                    print(print_item(result[0], grammar))
                    print(result[1])
                log.debug(f"Profile of work done: {chart.profile}")

def print_item(backpointers: Tuple[Item, List[Tuple]], gr: Grammar) -> str:
    item = backpointers[0]
    bps = backpointers[1]
    if not bps:
        terminal = item.rule.rhs
        return terminal
    lhs = item.rule.lhs
    rhs = item.rule.rhs
    parse = "(" + lhs
    ...
    for i in range(length):
        symbol = rhs[i]
        if gr.is_nonterminal(symbol):
            next_pointer = bps[i]
            parse = parse + " " + print_item(next_pointer, gr)
        else:
            parse = parse + " " + symbol
    parse = parse + ")"
    return parse
