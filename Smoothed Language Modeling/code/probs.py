...
class LanguageModel:
    ...
    def sample(self, x: Wordtype, y: Wordtype, max_length: int) -> str:
        # sample sentence using words in trigram model

        # if we reach max length, return "..."
        if max_length == 0:
            return "..."
        # get trigrams starting with xy and their frequencies
        trigrams = []
        freqs = []
        for ngram in self.event_count.keys():
            # only get ngrams of length 3, starting with xy, and where z is not "OOV"
            if len(ngram) == 3 and ngram[:2] == (x, y) and ngram[2] != "OOV":
                trigrams.append(ngram)
                freqs.append(self.event_count[ngram])
        # if there exists a trigram starting with xy, select a random trigram and get z
        z = ""
        if len(trigrams) != 0:
            # select a random trigram and get z
            random_trigram = random.choices(trigrams, freqs)[0]
            z = random_trigram[2]
        else:
            # in case there are no trigrams starting with xy, use bigrams starting with y
            bigrams = []
            freqs = []
            for ngram in self.event_count.keys():
                if len(ngram) == 2 and ngram[0] == y and ngram[1] != "OOV":
                    bigrams.append(ngram)
                    freqs.append(self.event_count[ngram])
            if len(bigrams) != 0:
                random_bigram = random.choices(bigrams, freqs)[0]
                z = random_bigram[1]
            else:
                # in case there are no bigrams starting with y, use unigrams
                unigrams = []
                freqs = []
                for ngram in self.event_count.keys():
                    if len(ngram) == 1 and ngram[0] != "OOV":
                        unigrams.append(ngram)
                        freqs.append(self.event_count[ngram])
                random_unigram = random.choices(unigrams, freqs)[0]
                z = random_unigram[0]
        ...
        # add z to return sentence and recursively call using yz and decremented max length
        return z + " " + self.sample(y, z, max_length - 1)
...
class BackoffAddLambdaLanguageModel(AddLambdaLanguageModel):
    ...
    def prob(self, x: Wordtype, y: Wordtype, z: Wordtype) -> float:
        # calculate probability of z given xy with backoff

        # probability of z
        num = self.event_count[z,] + self.lambda_
        den = self.context_count[()] + self.lambda_ * self.vocab_size
        probability_z = num / den

        # probability of z given y
        num = self.event_count[y, z] + self.lambda_ * self.vocab_size * probability_z
        den = self.context_count[y,] + self.lambda_ * self.vocab_size
        probability_z_given_y = num / den
        
        # probability of z given xy
        num = self.event_count[x, y, z] + self.lambda_ * self.vocab_size * probability_z_given_y
        den = self.context_count[x, y] + self.lambda_ * self.vocab_size
        probability_z_given_xy = num / den
        
        return probability_z_given_xy
...
class EmbeddingLogLinearLanguageModel(LanguageModel, nn.Module):
    ...
    def __init__(self, vocab: Vocab, lexicon_file: Path, l2: float, epochs: int) -> None:
        ...
        with open(lexicon_file) as file:
            ...
            self.embedding_matrix = torch.zeros((self.vocab_size, int(size[1])), dtype=torch.float32)
            self.unigram_count = torch.zeros((self.vocab_size), dtype=torch.float32)
            for line in file:
                line = line.split()
                word = line[0]
                line = line[1:]
                if word in self.vocab or word == OOL:
                    self.integerizer.add(word)
                    index = self.integerizer.index(word)
                    self.embedding_matrix[index] = torch.tensor([float(l) for l in line], dtype=torch.float32)

        self.dim: int = int(size[1])  # TODO: SET THIS TO THE DIMENSIONALITY OF THE VECTORS
    ...
    def log_prob_tensor(self, x: Wordtype, y: Wordtype, z: Wordtype) -> TorchScalar:
        ...
        # TODO: IMPLEMENT ME!
        x_idx = self.integerizer.index(x)
        y_idx = self.integerizer.index(y)
        z_idx = self.integerizer.index(z)
        
        if x_idx is None or x == OOV:
            x_idx = self.integerizer.index(OOL)
        if y_idx is None or y == OOV:
            y_idx = self.integerizer.index(OOL)
        if z_idx is None or z == OOV:
            z_idx = self.integerizer.index(OOL)

        x_embedding = self.embedding_matrix[x_idx].squeeze(0)
        y_embedding = self.embedding_matrix[y_idx].squeeze(0)
        z_embedding = self.embedding_matrix[z_idx].squeeze(0)

        logits = self.logits(x, y)
        Z = x_embedding @ self.X @ z_embedding.T + y_embedding @ self.Y @ z_embedding.T
        log_prob = Z - torch.logsumexp(logits, dim=-1)
        TorchScalar = TypeVar("TorchScalar", torch.Tensor, "log_prob")
        TorchScalar = log_prob
        return log_prob
    ...
    def logits(self, x: Wordtype, y: Wordtype) -> Float[torch.Tensor,"vocab"]:
        ...
        x_embedding = self.embedding_matrix[x_idx]
        y_embedding = self.embedding_matrix[y_idx]

        logits = x_embedding @ self.X @ self.embedding_matrix.T + y_embedding @ self.Y @ self.embedding_matrix.T
    ...
    def train(self, file: Path):    # type: ignore
        ...
        #     read_trigrams(file)
        #####################
        print("Training from corpus " + str(file))
        for epoch in range(self.epochs):
            F = 0
            for x, y, z in read_trigrams(file, self.vocab):
                log_prob = self.log_prob_tensor(x, y, z)
                regularizer = self.l2 * (torch.sum(self.X ** 2) + torch.sum(self.Y ** 2)) / N
                F_i = log_prob - regularizer
                (-F_i).backward()
                optimizer.step()
                optimizer.zero_grad()
                F += F_i
            print(f"epoch {epoch + 1}: F = {F / N}")
...
class ImprovedLogLinearLanguageModel(EmbeddingLogLinearLanguageModel):
    ...
    def log_prob_tensor(self, x: Wordtype, y: Wordtype, z: Wordtype) -> TorchScalar:
        ...
        unigram_prob = math.log(self.event_count[(z,)] + 1)

        # add unigram_prob to account for word frequencies in the training data
        Z = x_embedding @ self.X @ z_embedding.T + y_embedding @ self.Y @ z_embedding.T + unigram_prob
        log_prob = Z - torch.logsumexp(logits, dim=-1)
        TorchScalar = TypeVar("TorchScalar", torch.Tensor, "log_prob")
        TorchScalar = log_prob
        return log_prob
    ...
    def logits(self, x: Wordtype, y: Wordtype) -> Float[torch.Tensor,"vocab"]:
        ...
        # add unigram_count to account for word frequencies in the training data
        return logits + self.unigram_count
    ...
    def train(self, file: Path):
        ...
        # compute trigram counts in the training data
        for trigram in read_trigrams(file, self.vocab):
            self.count_trigram_events(trigram)
        
        # compute unigram log probabilities for all possible z in the vocab using training data word counts
        for word in self.vocab:
            index = self.integerizer.index(word)
            self.unigram_count[index] = math.log(self.event_count[word,] + 1)
