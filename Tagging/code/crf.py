...
class CRF(nn.Module):
    ...
    def log_forward(self, sentence: Sentence, corpus: TaggedCorpus) -> TorchScalar:
        ...
        for j in range(1, length - 1):
            w_j, t_j = sent[j]
            if t_j != None:
                alpha[j][t_j] = torch.logsumexp(alpha[j - 1] + torch.log(self.A[:, t_j]) + torch.log(self.B[t_j, w_j]), dim=0, keepdim=False, safe_inf=True)
            else:
                alpha[j] = torch.logsumexp(alpha[j - 1].reshape(-1, 1) + torch.log(self.A) + torch.log(self.B[:, w_j].reshape(1, -1)), dim=0, keepdim=False, safe_inf=True)
        log_Z = logsumexp_new(alpha[length - 2] + self.A[:, self.eos_t], dim=0, keepdim=False, safe_inf=True)
        return log_Z
    ...
    def viterbi_tagging(self, sentence: Sentence, corpus: TaggedCorpus) -> Sentence:
        ...
        for j in range(1, length - 1):
            alpha[j], backpointer[j] = torch.max(alpha[j - 1].reshape(-1, 1) + torch.log(self.A) + torch.log(self.B[:, sent[j][0]]).reshape(1, -1), 0) 
        alpha[length - 1], backpointer[length - 1] = torch.max(alpha[length - 2].reshape(-1, 1) + torch.log(self.A), 0)
        ...
        for j in reversed(range(length)):
            word = self.vocab[sent[j][0]]
            tag = self.tagset[prev_tag]
            prev_tag = backpointer[j][prev_tag]
            seq.append((word, tag))
        seq.reverse()
