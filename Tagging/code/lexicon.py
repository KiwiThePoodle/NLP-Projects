...
def log_counts_lexicon(corpus: TaggedCorpus) -> torch.Tensor:
    ...
    feature_matrix = torch.zeros((len(corpus.vocab), len(corpus.tagset)))
    tag_word_counts = defaultdict(lambda: defaultdict(int))
    for sentence in corpus.train_data:
        for word, tag in sentence:
            tag_word_counts[tag][word] += 1
    for word_index, word in enumerate(corpus.vocab):
        for tag_index, tag in enumerate(corpus.tagset):
            count = tag_word_counts[tag][word]
            feature_matrix[word_index, tag_index] = torch.log(1 + count)
...
def affixes_lexicon(corpus: TaggedCorpus) -> torch.Tensor:
    ...
    common_suffixes = ['ing', 'ed', 'ly', 'es', 's']
    feature_matrix = torch.zeros((len(corpus.vocab), len(common_suffixes)), dtype=torch.float32)
    for word_index, word in enumerate(corpus.vocab):
        for suffix_index, suffix in enumerate(common_suffixes):
            if word.endswith(suffix):
                feature_matrix[word_index, suffix_index] = 1.0
